...
class Peilbuis:
    def __init__(self, code, filt, report=False, url_lizard='https://vitens.lizard.net/api/v3/', proxydict={}):
        ...
        url_loc = '{}?code__icontains={}{}'.format(
            lizard_url_loc, code, str(filt).zfill(3))
        if self.report:
            print('GET', url_loc)
    ...
    def head_total(self, method='fill_no_diver_freq_1day_linear'):
        if self.uuid_hand != '' and self.uuid_diver != '':
            if 'fill_no_diver' in method:
                df_head = pandas.concat(
                    [self.head_diver, self.head_hand], axis=1)
                df_head['total'] = df_head['diver']
                df_head.loc[pandas.isnull(
                    df_head['diver']), 'total'] = df_head['hand']

                if method == 'fill_no_diver':
                    return df_head['total']
                elif method == 'fill_no_diver_freq_1day_linear':
                    h_tot = df_head['total'].resample(
                        '1D').mean().interpolate()
    ...
    def plot(self, stats=True, ax=None, **kwargs):
        ...
        if ax is None:
            ...
            fig.suptitle('{}, filter {} \n({}, {})'.format(
                self.code, self.filt, self.bkf, self.okf))
            head_total.plot(style='b-')
            if stats:
                plt.axhline(head_total.quantile(0.06), linestyle='--',
                            color='r', label='0.06 kwantiel')
                plt.axhline(head_total.quantile(0.50),
                            linestyle='--', color='g', label='0.50kwantiel')
                plt.axhline(head_total.quantile(0.94), linestyle='--',
                            color='orange', label='1.00 kwantiel')
            plt.legend()
            ...
            if stats:
                ax.axhline(head_total.quantile(0.06), linestyle='--',
                           color='r', label='0.06 kwantiel')
                ax.axhline(head_total.quantile(0.50), linestyle='--',
                           color='g', label='0.50kwantiel')
                ax.axhline(head_total.quantile(0.94), linestyle='--',
                           color='orange', label='1.00 kr--')
            return ax
    ...
    def __getattr__(self, name):
        if name == 'head_hand':
            self.head_hand = get_timeseries(
                self.uuid_hand, report=self.report).rename('hand')
            return self.head_hand
        elif name == 'head_diver':
            self.head_diver = get_timeseries(
                self.uuid_diver, report=self.report).rename('diver')
